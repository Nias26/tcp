...
int main(void) {
  ...
  // Send and receive messages ( TODO: Send https requests)
  // auto t_recv = std::async(std::launch::async, receive_bytes, sock, cSSl);
  // auto t_send = std::async(std::launch::async, send_bytes, sock, cSSl);
  // t_recv.wait();

  struct pollfd fds[2]{{0, POLLIN, 0}, {sock, POLLIN, 0}};
